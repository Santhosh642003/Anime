...
class TextTrack extends EventsTarget {
  constructor(init) {
    ...
    if (!IS_SERVER$1 && init.content) {
      import('media-captions').then(({ parseText, VTTCue, VTTRegion }) => {
        if (init.type === "json") {
          this.ab(init.content, VTTCue, VTTRegion);
        } else {
   ...
   */
  get readyState() {
    return this[TextTrackSymbol.ya];
  }
  get mode() {
    return this.Xa;
  }
  ...
  addCue(cue, trigger) {
    let i = 0, length = this.Ua.length;
    for (i = 0; i < length; i++)
      if (cue.endTime <= this.Ua[i].startTime)
        break;
    if (i === length)
      this.Ua.push(cue);
    else
      this.Ua.splice(i, 0, cue);
    if (trigger?.type !== "cuechange") {
      this[TextTrackSymbol._a]?.track.addCue(cue);
    }
    this.dispatchEvent(new DOMEvent("add-cue", { detail: cue, trigger }));
    if (isCueActive(cue, this.Va)) {
      this[TextTrackSymbol.Za](this.Va, trigger);
    }
  }
  removeCue(cue, trigger) {
    const index = this.Ua.indexOf(cue);
    if (index >= 0) {
      const isActive = this.Wa.includes(cue);
      this.Ua.splice(index, 1);
      this[TextTrackSymbol._a]?.track.removeCue(cue);
  ...
  setMode(mode, trigger) {
    ...
    this[TextTrackSymbol.Ta]?.();
  }
  /* @internal */
  [(_a = TextTrackSymbol.ya, _b = TextTrackSymbol.Ta, _c = TextTrackSymbol._a, TextTrackSymbol.Za)](currentTime, trigger) {
    this.Va = currentTime;
    if (this.mode === "disabled" || !this.Ua.length)
      return;
    const activeCues = [];
    for (let i = 0, length = this.Ua.length; i < length; i++) {
      const cue = this.Ua[i];
  ...
  async cb() {
    if (!this.Oa || !this.src || this[TextTrackSymbol.ya] > 0)
      return;
    this[TextTrackSymbol.ya] = 1;
    this.dispatchEvent(new DOMEvent("load-start"));
    try {
      const { parseResponse, VTTCue, VTTRegion } = await import('media-captions'), crossorigin = this[TextTrackSymbol.Ra]?.();
      const response = fetch(this.src, {
        headers: this.type === "json" ? { "Content-Type": "application/json" } : void 0,
        credentials: getRequestCredentials(crossorigin)
      });
      if (this.type === "json") {
        this.ab(await (await response).text(), VTTCue, VTTRegion);
      } else {
  ...
  ya() {
    ...
    if (!this.src || this.type !== "vtt") {
      const nativeTrack = this[TextTrackSymbol._a]?.track;
      if (nativeTrack)
        for (const cue of this.Ua)
          nativeTrack.addCue(cue);
    }
    const loadEvent = new DOMEvent("load");
    this[TextTrackSymbol.Za](this.Va, loadEvent);
    this.dispatchEvent(loadEvent);
  }
  db(error) {
    this[TextTrackSymbol.ya] = 3;
    this.dispatchEvent(new DOMEvent("error", { detail: error }));
  }
  ab(json, VTTCue, VTTRegion) {
    try {
...
function getClippingRect(_ref) {
  ...
  const elementClippingAncestors = boundary === "clippingAncestors" ? getClippingElementAncestors(element, this.Mi) : [].concat(boundary);
...
class NativeTextRenderer {
  ...
  removeTrack(track) {
    track[TextTrackSymbol._a]?.remove?.();
    track[TextTrackSymbol._a] = null;
    this.bg.delete(track);
  }
  changeTrack(track) {
    const current = track?.[TextTrackSymbol._a];
  ...
  detach() {
    if (this.Da)
      this.Da.textTracks.onchange = null;
  ...
  eg(track) {
    if (!this.Da)
      return;
    const el = track[TextTrackSymbol._a] ??= this.fg(track);
  ...
  oa(event) {
    for (const track of this.bg) {
      const nativeTrack = track[TextTrackSymbol._a]?.track;
      if (!nativeTrack)
        continue;
      if (!this.cg) {
...
class TextRenderers {
  constructor(_media) {
    ...
    listenEvent(textTracks, "add", this.Ja.bind(this));
    listenEvent(textTracks, "remove", this.Ka.bind(this));
    listenEvent(textTracks, "mode-change", this.Ba.bind(this));
  }
  Ia() {
    const { $state, $iosControls } = this.Ha;
    this.Ea = $state.controls() || $iosControls();
    this.Ba();
  }
  add(renderer) {
    this.Ca.push(renderer);
    this.Ba();
  }
  remove(renderer) {
    renderer.detach();
    this.Ca.splice(this.Ca.indexOf(renderer), 1);
    this.Ba();
  }
  /* @internal */
  Ma(video) {
    requestAnimationFrame(() => {
      this.Da = video;
      if (video) {
        this.za = new NativeTextRenderer();
        this.za.attach(video);
        for (const track of this.Fa)
          this.Ga(track);
      }
      this.Ba();
    });
  ...
  Ga(track) {
    if (!isTrackCaptionKind(track))
      return;
    this.za?.addTrack(track);
  }
  La(track) {
    if (!isTrackCaptionKind(track))
      return;
    this.za?.removeTrack(track);
  }
  Ja(event) {
    this.Ga(event.detail);
  }
  Ka(event) {
    this.La(event.detail);
  }
  Ba() {
    if (!this.Da) {
      this.P();
    ...
    const currentTrack = this.Fa.selected;
    if (this.Ea || currentTrack?.[TextTrackSymbol.Na]) {
      this.Aa?.changeTrack(null);
      this.za?.setDisplay(true);
      this.za?.changeTrack(currentTrack);
      return;
    }
    this.za?.setDisplay(false);
    this.za?.changeTrack(null);
    if (!currentTrack) {
      this.Aa?.changeTrack(null);
      return;
    }
    const customRenderer = this.Ca.sort((a, b) => a.priority - b.priority).find((loader) => loader.canRender(currentTrack));
    if (this.Aa !== customRenderer) {
      this.Aa?.detach();
      customRenderer?.attach(this.Da);
      this.Aa = customRenderer ?? null;
    }
...
class TextTrackList extends List {
  ...
  add(init, trigger) {
    ...
    track[TextTrackSymbol.Ra] = this[TextTrackSymbol.Ra];
    if (this.Oa)
      track[TextTrackSymbol.Oa]();
    if (init.default) {
...
class MediaKeyboardController extends MediaPlayerController {
  ...
  ub() {
    ...
    effect(() => {
      if (!$active())
        return;
      listenEvent(target, "keyup", this.vb.bind(this));
      listenEvent(target, "keydown", this.wb.bind(this));
      listenEvent(target, "keydown", this.xb.bind(this), { capture: true });
    });
  }
  vb(event) {
    ...
    let { method, value } = this.rb(event);
    ...
    if (method?.startsWith("seek")) {
      ...
      if (this.qb) {
        this.tb(event, method === "seekForward");
        this.qb = null;
      } else {
        this.Ha.remote.seek(this.sb, event);
        this.sb = void 0;
      }
    }
  ...
  wb(event) {
    ...
    if (!method && !event.metaKey && /[0-9]/.test(event.key)) {
      event.preventDefault();
      event.stopPropagation();
      this.Ha.remote.seek(this.$state.duration() / 10 * Number(event.key), event);
      return;
    }
    if (!method)
      return;
    ...
    switch (method) {
      case "seekForward":
      case "seekBackward":
        this.yb(event, method, method === "seekForward");
        break;
      case "volumeUp":
      case "volumeDown":
        const volumeSlider = this.el.querySelector("[data-media-volume-slider]");
      ...
      case "toggleFullscreen":
        this.Ha.remote.toggleFullscreen("prefer-media", event);
        break;
      case "speedUp":
      case "slowDown":
        const playbackRate = this.$state.playbackRate();
        this.Ha.remote.changePlaybackRate(
          Math.max(0.25, Math.min(2, playbackRate + (method === "speedUp" ? 0.25 : -0.25))),
          event
        );
        break;
      default:
        this.Ha.remote[method]?.(event);
    }
  }
  xb(event) {
    if (isHTMLMediaElement(event.target) && this.rb(event).method) {
      event.preventDefault();
    }
  }
  rb(event) {
  ...
  zb(event, type) {
    ...
    return this.sb = Math.max(
      0,
      Math.min(
        (this.sb ?? this.$state.currentTime()) + (type === "seekForward" ? +seekBy : -seekBy),
        this.$state.duration()
      )
    );
  }
  tb(event, forward) {
    this.qb?.dispatchEvent(
  ...
  yb(event, type, forward) {
    if (!this.$state.canSeek())
      return;
    if (!this.qb)
      this.qb = this.el.querySelector("[data-media-time-slider]");
    if (this.qb) {
      this.tb(event, forward);
    } else {
      this.Ha.remote.seeking(this.zb(event, type), event);
    }
  }
}
...
class AudioProviderLoader {
  ...
  async load() {
    ...
    return new (await import('./vidstack-c6290e1a.js')).AudioProvider(this.target);
  }
}
...
class VideoProviderLoader {
  ...
  async load(context) {
    ...
    return new (await import('./vidstack-335f14e5.js')).VideoProvider(this.target, context);
  }
}
...
const _HLSProviderLoader = class _HLSProviderLoader2 extends VideoProviderLoader {
  ...
  async load(context) {
    ...
    return new (await import('./vidstack-412a7593.js')).HLSProvider(this.target, context);
  }
};
...
class MediaPlayerDelegate {
  ...
  Nb(type, ...init) {
    if (IS_SERVER$1)
      return;
    this.oc(new DOMEvent(type, init?.[0]));
  }
  async Dg(info, trigger) {
    if (IS_SERVER$1)
      return;
    const { $state, logger } = this.Ha;
    if (peek($state.canPlay))
      return;
    this.Nb("can-play", {
      detail: {
        ...info,
        provider: peek(this.Ha.$provider)
      },
      trigger
    });
    tick();
...
class Queue {
  ...
  ia(key, item) {
    if (!this.Pg.has(key))
      this.Pg.set(key, /* @__PURE__ */ new Set());
    this.Pg.get(key).add(item);
  }
  /**
   * Process all items in queue for the given `key`.
   */
  ih(key, callback) {
    const items = this.Pg.get(key);
    if (items)
...
class MediaRequestManager extends MediaPlayerController {
  constructor(_stateMgr, _request, _media) {
    ...
    this.Ha = _media;
    this.Rg = false;
    this.Rb = _media.$provider;
    this.rc = new MediaControls();
    this.Sg = new FullscreenController();
  ...
  onConnect() {
    const names = Object.getOwnPropertyNames(Object.getPrototypeOf(this)), handle = this.Yg.bind(this);
  ...
  async mc(trigger) {
    ...
    this.Og.Pg.ia("play", trigger);
    try {
      const provider = peek(this.Rb);
  ...
  async sc(target = "prefer-media", trigger) {
    if (IS_SERVER$1)
      return;
    const adapter = this.Vg(target);
    throwIfFullscreenNotSupported(target, adapter);
    if (adapter.active)
      return;
    if (peek(this.$state.pictureInPicture)) {
      this.Rg = true;
      await this.vc(trigger);
    }
    ...
    this.Og.Pg.ia("fullscreen", trigger);
    return adapter.enter();
  }
  async tc(target = "prefer-media", trigger) {
    if (IS_SERVER$1)
      return;
    const adapter = this.Vg(target);
    throwIfFullscreenNotSupported(target, adapter);
    ...
    try {
      ...
      if (this.Rg && peek(this.$state.canPictureInPicture)) {
        await this.uc();
      }
      return result;
    } finally {
  ...
  Vg(target) {
    const provider = peek(this.Rb);
    return target === "prefer-media" && this.Sg.supported || target === "media" ? this.Sg : provider?.fullscreen;
  }
  async uc(trigger) {
    if (IS_SERVER$1)
      return;
    ...
    this.Og.Pg.ia("pip", trigger);
    return await this.Rb().pictureInPicture.enter();
  }
  async vc(trigger) {
    if (IS_SERVER$1)
      return;
    ...
    this.Og.Pg.ia("pip", trigger);
    return await this.Rb().pictureInPicture.exit();
  }
  Wg() {
  ...
  Zg() {
    this.rc.defaultDelay = this.$props.controlsDelay();
  }
  _g() {
    const { canLoad, canFullscreen } = this.$state, supported = this.Sg.supported || this.Rb()?.fullscreen?.supported || false;
    if (canLoad() && peek(canFullscreen) === supported)
      return;
    canFullscreen.set(supported);
  }
  $g() {
    const { canLoad, canPictureInPicture } = this.$state, supported = this.Rb()?.pictureInPicture?.supported || false;
    if (canLoad() && peek(canPictureInPicture) === supported)
      return;
    canPictureInPicture.set(supported);
  }
  ["media-audio-track-change-request"](event) {
    if (this.Ha.audioTracks.readonly) {
      return;
    }
    const index = event.detail, track = this.Ha.audioTracks[index];
    if (track) {
      this.Og.Pg.ia("audioTrack", event);
      track.selected = true;
    }
  }
  async ["media-enter-fullscreen-request"](event) {
    try {
      await this.sc(event.detail, event);
    } catch (error) {
      this.o(error, event);
    }
  }
  async ["media-exit-fullscreen-request"](event) {
    try {
      await this.tc(event.detail, event);
    } catch (error) {
      this.o(error, event);
    }
  }
  ...
  async ["media-orientation-lock-request"](event) {
    try {
      this.Og.Pg.ia("orientation", event);
      await this.bd.lock(event.detail);
    } catch (error) {
      this.Og.Pg.Tg("orientation");
    }
  }
  ...
  ["media-rate-change-request"](event) {
    if (this.$state.playbackRate() === event.detail)
      return;
    this.Og.Pg.ia("rate", event);
    this.Rb().playbackRate = event.detail;
  }
  ["media-quality-change-request"](event) {
    if (this.Ha.qualities.readonly) {
      return;
    }
    this.Og.Pg.ia("quality", event);
    const index = event.detail;
    if (index < 0) {
      this.Ha.qualities.autoSelect(event);
    } else {
      const quality = this.Ha.qualities[index];
      if (quality) {
  ...
  ["media-pause-controls-request"](event) {
    this.Og.Pg.ia("controls", event);
    this.rc.pause(event);
  }
  ["media-resume-controls-request"](event) {
    this.Og.Pg.ia("controls", event);
    this.rc.resume(event);
  }
  ...
  ["media-start-loading"](event) {
    if (this.$state.canLoad())
      return;
    this.Og.Pg.ia("load", event);
    this.Tb.oc(this.createEvent("can-load"));
  }
  ["media-text-track-change-request"](event) {
    const { index, mode } = event.detail, track = this.Ha.textTracks[index];
    if (track) {
      this.Og.Pg.ia("textTrack", event);
...
class MediaStateManager extends MediaPlayerController {
  constructor(_request, _media) {
    ...
    this.sh = functionDebounce(() => {
      ...
      waiting.set(true);
      playing.set(false);
      const event = this.createEvent("waiting", { trigger: this.qh });
      this.mh.set("waiting", event);
      this.dispatch(event);
      this.qh = void 0;
      this.rh = false;
    }, 300);
  ...
  Dh() {
    if (!this.ph)
      return;
    if (this.Ha.$provider()?.paused) {
      requestAnimationFrame(() => {
        if (!this.scope)
          return;
        this.Ha.remote.play(new DOMEvent("dom-connect"));
      });
    }
    this.ph = false;
  }
  Eh() {
    if (this.ph)
      return;
    this.ph = !this.Ha.$state.paused();
    this.Ha.$provider()?.pause();
  }
  oh() {
    this.uh();
    this.Og.Qg = false;
  ...
  nh(request, event) {
    this.Og.Pg.ih(request, (requestEvent) => {
  ...
  Ah() {
    this.th();
    this.vh();
    const textTracks = this.Ha.textTracks;
    listenEvent(textTracks, "add", this.th.bind(this));
    listenEvent(textTracks, "remove", this.th.bind(this));
    listenEvent(textTracks, "mode-change", this.vh.bind(this));
  }
  Bh() {
    const qualities = this.Ha.qualities;
    listenEvent(qualities, "add", this.wh.bind(this));
    listenEvent(qualities, "remove", this.wh.bind(this));
    listenEvent(qualities, "change", this.Fh.bind(this));
    listenEvent(qualities, "auto-change", this.Gh.bind(this));
    listenEvent(qualities, "readonly-change", this.Hh.bind(this));
  }
  Ch() {
    const audioTracks = this.Ha.audioTracks;
    listenEvent(audioTracks, "add", this.xh.bind(this));
    listenEvent(audioTracks, "remove", this.xh.bind(this));
    listenEvent(audioTracks, "change", this.Ih.bind(this));
  }
  th(event) {
    const { textTracks } = this.$state;
    textTracks.set(this.Ha.textTracks.toArray());
  ...
  vh(event) {
    if (event)
      this.nh("textTrack", event);
    const current = this.Ha.textTracks.selected, { textTrack } = this.$state;
  ...
  xh(event) {
    ...
    audioTracks.set(this.Ha.audioTracks.toArray());
  ...
  Ih(event) {
    ...
    audioTrack.set(this.Ha.audioTracks.selected);
    this.nh("audioTrack", event);
  ...
  wh(event) {
    ...
    qualities.set(this.Ha.qualities.toArray());
  ...
  Fh(event) {
    ...
    quality.set(this.Ha.qualities.selected);
    this.nh("quality", event);
  ...
  Gh() {
    this.$state.autoQuality.set(this.Ha.qualities.auto);
  }
  Hh() {
    this.$state.canSetQuality.set(!this.Ha.qualities.readonly);
  }
  ["provider-change"](event) {
    const prevProvider = this.Ha.$provider(), newProvider = event.detail;
    if (prevProvider?.type === newProvider?.type)
      return;
    prevProvider?.destroy?.();
    prevProvider?.scope?.dispose();
    this.Ha.$provider.set(event.detail);
  ...
  ["can-load"](event) {
    ...
    this.mh.set("can-load", event);
    this.nh("load", event);
    this.Ha.textTracks[TextTrackSymbol.Oa]();
  }
  ["media-type-change"](event) {
    appendTriggerEvent(event, this.mh.get("source-change"));
    const viewType = peek(this.$state.viewType);
    this.$state.mediaType.set(event.detail);
    const providedViewType = peek(this.$state.providedViewType), currentViewType = providedViewType === "unknown" ? event.detail : providedViewType;
  ...
  ["source-change"](event) {
    appendTriggerEvent(event, this.mh.get("sources-change"));
    this.yh(event);
    this.mh.set(event.type, event);
    this.$state.source.set(event.detail);
    this.el?.setAttribute("aria-busy", "true");
  }
  yh(event) {
    this.Ha.audioTracks[ListSymbol.j](event);
    this.Ha.qualities[ListSymbol.j](event);
    this.oh();
    softResetMediaState(this.Ha.$state);
  }
  ["abort"](event) {
    const sourceChangeEvent = this.mh.get("source-change");
    appendTriggerEvent(event, sourceChangeEvent);
    if (!sourceChangeEvent?.trigger) {
      appendTriggerEvent(event, this.mh.get("can-load"));
    }
  }
  ["load-start"](event) {
    appendTriggerEvent(event, this.mh.get("source-change"));
  ...
  ["error"](event) {
    ...
    appendTriggerEvent(event, this.mh.get("abort"));
  }
  ["loaded-metadata"](event) {
    appendTriggerEvent(event, this.mh.get("load-start"));
  }
  ["loaded-data"](event) {
    appendTriggerEvent(event, this.mh.get("load-start"));
  }
  ["can-play"](event) {
    if (event.trigger?.type !== "loadedmetadata") {
      appendTriggerEvent(event, this.mh.get("loaded-metadata"));
    }
    this.zh(event.detail);
    this.el?.setAttribute("aria-busy", "false");
  }
  ["can-play-through"](event) {
    this.zh(event.detail);
    appendTriggerEvent(event, this.mh.get("can-play"));
  }
  zh(detail) {
  ...
  ["play"](event) {
    ...
    if (this.Og.Ug || !paused()) {
      event.stopImmediatePropagation();
      return;
    }
    appendTriggerEvent(event, this.mh.get("waiting"));
    this.nh("play", event);
    this.mh.set("play", event);
    paused.set(false);
    ...
    if (ended() || this.Og.Qg) {
      this.Og.Qg = false;
      ended.set(false);
      this.oc(this.createEvent("replay", { trigger: event }));
    }
    if (!playsinline() && pointer() === "coarse") {
      this.Ha.remote.enterFullscreen("prefer-media", event);
    }
  }
  ["play-fail"](event) {
    const { muted, autoplaying } = this.$state;
    appendTriggerEvent(event, this.mh.get("play"));
    this.nh("play", event);
    const { paused, playing } = this.$state;
    paused.set(true);
    playing.set(false);
    this.oh();
    this.mh.set("play-fail", event);
    if (autoplaying()) {
      this.oc(
  ...
  ["playing"](event) {
    const playEvent = this.mh.get("play");
    if (playEvent) {
      appendTriggerEvent(event, this.mh.get("waiting"));
      appendTriggerEvent(event, playEvent);
    } else {
      appendTriggerEvent(event, this.mh.get("seeked"));
    }
    setTimeout(() => this.oh(), 0);
    const {
      paused,
      playing,
      live,
    ...
    if (live() && !started() && currentTime() === 0) {
      ...
      if (Number.isFinite(end))
        this.Ha.$provider().currentTime = end;
    }
    this["started"](event);
  }
  ["started"](event) {
    ...
    if (!started()) {
      started.set(true);
      this.oc(this.createEvent("started", { trigger: event }));
    }
  }
  ["pause"](event) {
  ...
  ["time-update"](event) {
    ...
    waiting.set(false);
    for (const track of this.Ha.textTracks) {
      track[TextTrackSymbol.Za](detail.currentTime, event);
    }
  }
  ["volume-change"](event) {
  ...
  ["seeked"](event) {
    ...
    if (this.Og.yb) {
      ...
      const waitingEvent = this.mh.get("waiting");
      appendTriggerEvent(event, waitingEvent);
      if (waitingEvent?.trigger?.type !== "seeking") {
        appendTriggerEvent(event, this.mh.get("seeking"));
      }
      if (paused())
        this.uh();
      seeking.set(false);
  ...
  ["fullscreen-change"](event) {
    ...
    this.nh("fullscreen", event);
  }
  ["fullscreen-error"](event) {
    this.nh("fullscreen", event);
  }
  ["orientation-change"](event) {
    this.nh("orientation", event);
  }
...
const _MediaPlayer = class _MediaPlayer2 extends Component {
  constructor() {
    ...
    context.$iosControls = computed(this.cc.bind(this));
    context.textTracks = new TextTrackList();
    context.textTracks[TextTrackSymbol.Ra] = this.$state.crossorigin;
    ...
    this.Tb = new MediaStateManager(request, context);
    this.Sb = new MediaRequestManager(this.Tb, request, context);
    context.delegate = new MediaPlayerDelegate(
      this.Tb.oc.bind(this.Tb),
      context
    );
    new MediaLoadController(this.startLoading.bind(this));
  }
  get Rb() {
    return this.Ha.$provider();
  }
  onSetup() {
    this.dc();
  ...
  onAttach(el) {
    ...
    listenEvent(el, "find-media-player", this.kc.bind(this));
  }
  onConnect(el) {
    if (IS_IPHONE)
    ...
    this.Wb(pointerQuery);
    pointerQuery.onchange = this.Wb.bind(this);
  ...
  ec() {
    if (this.$state.canPlay() && this.Rb)
      this.canPlayQueue.pc();
    else
      this.canPlayQueue.qc();
  }
  Xb() {
  ...
  get qualities() {
    return this.Ha.qualities;
  }
  get audioTracks() {
    return this.Ha.audioTracks;
  }
  get textTracks() {
    return this.Ha.textTracks;
  }
  get textRenderers() {
    return this.Ha.textRenderers;
  }
  get paused() {
    return this.Rb?.paused ?? true;
  }
  set paused(paused) {
    this.Yb(paused);
  }
  pa() {
    this.Yb(this.$props.paused());
  }
  Yb(paused) {
    if (paused) {
      this.canPlayQueue.ia("paused", () => this.Sb.lc());
    } else
      this.canPlayQueue.ia("paused", () => this.Sb.mc());
  }
  get muted() {
    return this.Rb?.muted ?? false;
  }
  set muted(muted) {
    this.Zb(muted);
  }
  fc() {
    this.Zb(this.$props.muted());
  }
  Zb(muted) {
    this.canPlayQueue.ia("muted", () => this.Rb.muted = muted);
  }
  get currentTime() {
    return this.Rb?.currentTime ?? 0;
  }
  set currentTime(time) {
    this._b(time);
  }
  hc() {
    this._b(this.$props.currentTime());
  }
  _b(time) {
    this.canPlayQueue.ia("currentTime", () => {
      const adapter = this.Rb;
  ...
  get volume() {
    return this.Rb?.volume ?? 1;
  }
  set volume(volume) {
    this.$b(volume);
  }
  gc() {
    this.$b(this.$props.volume());
  }
  $b(volume) {
    const clampedVolume = clampNumber(0, volume, 1);
    this.canPlayQueue.ia("volume", () => this.Rb.volume = clampedVolume);
  }
  get playbackRate() {
    return this.Rb?.playbackRate ?? 1;
  }
  set playbackRate(rate) {
    this.ac(rate);
  }
  jc() {
    this.ac(this.$props.playbackRate());
  }
  ac(rate) {
    this.canPlayQueue.ia("rate", () => this.Rb.playbackRate = rate);
  }
  ic() {
    this.nc(this.$props.playsinline());
  }
  nc(inline) {
    this.canPlayQueue.ia("playsinline", () => this.Rb.playsinline = inline);
  }
  async play(trigger) {
    return this.Sb.mc(trigger);
  }
  async pause(trigger) {
    return this.Sb.lc(trigger);
  }
  async enterFullscreen(target, trigger) {
    return this.Sb.sc(target, trigger);
  }
  async exitFullscreen(target, trigger) {
    return this.Sb.tc(target, trigger);
  }
  enterPictureInPicture(trigger) {
    return this.Sb.uc(trigger);
  }
  exitPictureInPicture(trigger) {
    return this.Sb.vc(trigger);
  }
  seekToLiveEdge(trigger) {
    this.Sb.wc(trigger);
  }
  startLoading(trigger) {
    this.Ha.delegate.Nb("can-load", { trigger });
  }
  matchQuery(query) {
    return scoped(() => PlayerQueryList.create(query), this.scope);
  }
  destroy() {
    this.Ha.remote.setPlayer(null);
...
const SETUP = Symbol(0), sourceTypes = /* @__PURE__ */ new Map();
class SourceSelection {
  constructor(_domSources, _media, _loader) {
    ...
    for (const src of $state.sources()) {
      const loader = this.lg().find((loader2) => loader2.canPlay(src));
      if (!loader)
        continue;
      const mediaType = loader.mediaType(src);
      this.Ha.$state.source.set(src);
      this.Ha.$state.mediaType.set(mediaType);
      this.Ha.$state.inferredViewType.set(mediaType);
      this.Eb.set(loader);
      this.kg = true;
    }
  }
  connect() {
    ...
    if (this.kg) {
      this.mg(this.Ha.$state.source(), loader);
      this.ng(loader);
      this.kg = false;
    }
  ...
  pg() {
    this.Ha.delegate.Nb("sources-change", {
      detail: [...normalizeSrc(this.Ha.$props.src()), ...this.Gb()]
    });
  }
  qg() {
    const { $state } = this.Ha;
    const sources = $state.sources(), currentSource = peek($state.source), newSource = this.og(currentSource, sources), noMatch = sources[0]?.src && !newSource.src && !newSource.type;
    if (noMatch) {
      ...
      Promise.all(
        ...
        if (abort.signal.aborted)
          return;
        this.og(peek($state.source), sources2);
        tick();
      });
      return () => abort.abort();
    }
  ...
  og(currentSource, sources) {
    let newSource = { src: "", type: "" }, newLoader = null;
    for (const src of sources) {
      const loader = peek(this.lg).find((loader2) => loader2.canPlay(src));
      if (loader) {
        newSource = src;
        newLoader = loader;
      }
  ...
  mg(src, loader) {
    this.Ha.delegate.Nb("source-change", { detail: src });
    this.Ha.delegate.Nb("media-type-change", {
      detail: loader?.mediaType(src) || "unknown"
    });
  }
  ng(loader) {
    this.Ha.delegate.Nb("provider-change", { detail: null });
    loader && peek(() => loader.preconnect?.(this.Ha));
    this.Eb.set(loader);
    this.Ha.delegate.Nb("provider-loader-change", { detail: loader });
  }
  rg() {
    const provider = this.Ha.$provider();
    if (!provider || provider[SETUP])
      return;
    if (this.Ha.$state.canLoad()) {
      scoped(() => provider.setup(this.Ha), provider.scope);
      provider[SETUP] = true;
      return;
    }
    peek(() => provider.preconnect?.(this.Ha));
  }
  sg() {
    const provider = this.Ha.$provider(), source = this.Ha.$state.source(), crossorigin = peek(this.Ha.$state.crossorigin);
    ...
    if (this.Ha.$state.canLoad()) {
      ...
      if (isHLSSrc(source)) {
        if (!isHLSSupported()) {
          resolveStreamTypeFromHLSManifest(source.src, {
            ...
            this.Ha.delegate.Nb("stream-type-change", {
              detail: streamType
            });
          }).catch(noop);
        }
      ...
      peek(() => provider?.loadSource(source, peek(this.Ha.$state.preload)));
      return () => abort.abort();
    }
    try {
...
function normalizeSrc(src) {
  return (isArray(src) ? src : [!isString(src) && "src" in src ? src : { src }]).map(
    ({ src: src2, type }) => ({
      src: src2,
      type: type ?? (isString(src2) ? sourceTypes.get(src2) : null) ?? (!isString(src2) || src2.startsWith("blob:") ? "video/object" : "?")
    })
  );
}
...
class Tracks {
  ...
  ug() {
    ...
    for (const oldTrack of this.tg) {
      if (!newTracks.some((t) => t.id === oldTrack.id)) {
        const track = oldTrack.id && this.Ha.textTracks.getById(oldTrack.id);
        if (track)
          this.Ha.textTracks.remove(track);
      }
    }
    for (const newTrack of newTracks) {
      const id = newTrack.id || TextTrack.createId(newTrack);
      if (!this.Ha.textTracks.getById(id)) {
        newTrack.id = id;
        this.Ha.textTracks.add(newTrack);
      }
    }
    this.tg = newTracks;
  }
...
class MediaProvider extends Component {
  ...
  onSetup() {
    this.Ha = useMediaContext();
    this.Lb = new SourceSelection(this.Gb, this.Ha, this.$state.loader);
  }
  onAttach(el) {
  ...
  Mb(target) {
    if (!this.scope)
      return;
    const loader = this.$state.loader(), { $provider } = this.Ha;
    ...
    if (!loader || !target)
      return;
    loader.load(this.Ha).then((provider) => {
      if (!this.scope)
        return;
      ...
      if (provider) {
        peek(() => {
          const { muted, volume, playsinline } = this.Ha.$state;
          provider.muted = muted();
          provider.volume = volume();
          provider.playsinline = playsinline();
        });
      }
      this.Ha.delegate.Nb("provider-change", {
        detail: provider
      });
    });
  }
  ...
  Ib() {
    if (!this.el)
      return;
    const player = this.Ha.player, width = this.el.offsetWidth, height = this.el.offsetHeight;
    if (!player)
      return;
...
class Controls extends Component {
  ...
  onAttach(el) {
    const { pictureInPicture, fullscreen } = this.Ha.$state;
  ...
  Db() {
    if (!this.el)
      return;
    const { $iosControls } = this.Ha, { controls } = this.Ha.$state, isHidden = controls() || $iosControls();
    setAttribute(this.el, "aria-hidden", isHidden ? "true" : null);
    setStyle(this.el, "display", isHidden ? "none" : null);
  }
  Cb() {
    const { controls } = this.Ha.player, { hideDelay } = this.$props;
    controls.defaultDelay = hideDelay() === 2e3 ? this.Ha.$props.controlsDelay() : hideDelay();
  }
  Bb() {
    const { controlsVisible } = this.Ha.$state;
...
class Popper extends ViewController {
  ...
  show(trigger) {
    ...
    this.Kh = window.setTimeout(
      () => {
        this.Kh = -1;
        const content = this.Xc.yc();
        if (content)
          content.style.removeProperty("display");
        peek(() => this.Xc.oa(true, trigger));
      },
      this.Xc.Dc?.() ?? 0
    );
  }
...
class Tooltip extends Component {
  ...
  Qb(el) {
    ...
    setAttribute(el, "data-describedby", this.zc);
  }
  Pb(el) {
  ...
  Cc(isShowing) {
    const trigger = this.xc(), content = this.yc();
    if (trigger) {
      setAttribute(trigger, "aria-describedby", isShowing ? this.zc : null);
    }
...
class TooltipContent extends Component {
  ...
  Fc() {
    ...
    return autoPlacement(this.el, this.Gc(), placement(), {
  ...
  Gc() {
    return useContext(tooltipContext).xc();
  }
}
...
class ToggleButtonController extends ViewController {
  ...
  Nh() {
    return ariaBool$1(this.Xc.Ic());
  }
  Ph(event) {
    if (isWriteSignal(this.Xc.Ic)) {
      this.Xc.Ic.set((p) => !p);
    }
  }
  Oh(event) {
    ...
    event.preventDefault();
    (this.Xc.Jc ?? this.Ph).call(this, event);
  }
}
...
let PlayButton$1 = class PlayButton extends Component {
  ...
  onSetup() {
    this.Ha = useMediaContext();
    const { paused, ended } = this.Ha.$state;
    this.setAttributes({
      "data-paused": paused,
      "data-ended": ended
    });
  ...
  Jc(event) {
    const remote = this.Ha.remote;
    this.Ic() ? remote.pause(event) : remote.play(event);
  }
  Ic() {
    const { paused } = this.Ha.$state;
    return !paused();
  }
  Kc() {
    const { paused } = this.Ha.$state;
...
let CaptionButton$1 = class CaptionButton extends Component {
  ...
  Jc(event) {
    this.Ha.remote.toggleCaptions(event);
  }
  Ic() {
    const { textTrack } = this.Ha.$state, track = textTrack();
    return !!track && isTrackCaptionKind(track);
  }
  Mc() {
    const { textTracks } = this.Ha.$state;
    return textTracks().filter(isTrackCaptionKind).length == 0;
  }
  Kc() {
    const { textTrack } = this.Ha.$state;
...
let FullscreenButton$1 = class FullscreenButton extends Component {
  ...
  onSetup() {
    this.Ha = useMediaContext();
    const { fullscreen } = this.Ha.$state, isSupported = this.Nc.bind(this);
    this.setAttributes({
  ...
  Jc(event) {
    const remote = this.Ha.remote, target = this.$props.target();
    this.Ic() ? remote.exitFullscreen(target, event) : remote.enterFullscreen(target, event);
  }
  Ic() {
    const { fullscreen } = this.Ha.$state;
    return fullscreen();
  }
  Nc() {
    const { canFullscreen } = this.Ha.$state;
...
let MuteButton$1 = class MuteButton extends Component {
  ...
  Ic() {
    const { muted, volume } = this.Ha.$state;
  ...
  Oc() {
    const { muted, volume } = this.Ha.$state, $volume = volume();
    if (muted() || $volume === 0)
      return "muted";
    else if ($volume >= 0.5)
      return "high";
...
let PIPButton$1 = class PIPButton extends Component {
  ...
  onSetup() {
    this.Ha = useMediaContext();
    const { pictureInPicture } = this.Ha.$state, isSupported = this.Nc.bind(this);
    this.setAttributes({
  ...
  Ic() {
    const { pictureInPicture } = this.Ha.$state;
    return pictureInPicture();
  }
  Nc() {
    const { canPictureInPicture } = this.Ha.$state;
...
let SeekButton$1 = class SeekButton extends Component {
  ...
  onSetup() {
    this.Ha = useMediaContext();
    const { seeking } = this.Ha.$state, { seconds } = this.$props, isSupported = this.Nc.bind(this);
    this.setAttributes({
      seconds,
  ...
  Jc(event) {
    ...
    if (disabled())
      return;
    const { currentTime } = this.Ha.$state, seekTo = currentTime() + seconds();
    this.Ha.remote.seek(seekTo, event);
  }
};
SeekButton$1.props = {
  disabled: false,
...
let LiveButton$1 = class LiveButton extends Component {
  ...
  onSetup() {
    this.Ha = useMediaContext();
    const { disabled } = this.$props, { live, liveEdge } = this.Ha.$state, isHidden = () => !live();
    this.setAttributes({
      "data-edge": liveEdge,
  ...
  Jc(event) {
    const { disabled } = this.$props, { liveEdge } = this.Ha.$state;
    if (disabled() || liveEdge())
      return;
    this.Ha.remote.seekToLiveEdge(event);
  }
};
LiveButton$1.props = {
  disabled: false
...
class SliderEventsController extends ViewController {
  ...
  onConnect() {
    ...
    if (this.Xc.Kd) {
      const provider = this.Ha.player.el?.querySelector(
  ...
  _h() {
    ...
    this.listen("keydown", this.wb.bind(this));
    this.listen("keyup", this.vb.bind(this));
  }
  $h() {
    if (this.Xc.Wc() || !this.$state.dragging())
      return;
  ...
  Th(event) {
    ...
    if (this.$props.orientation() === "vertical") {
      ...
      if (this.Qh && isNumber(this.Sh)) {
        const { width } = this.Rb.getBoundingClientRect(), rate = (event.clientX - this.Qh.clientX) / width, range = max() - min(), diff = range * Math.abs(rate);
    ...
    return Math.max(
      ...
      Math.min(
        max(),
        this.Xc.Vc(
          getValueFromRate(min(), max(), thumbPositionRate, this.Xc.Tc())
        )
      )
    );
  }
  ...
  Uh(value, trigger) {
    ...
    if (dragging())
      return;
    dragging.set(true);
    this.Ha.remote.pauseControls(trigger);
  ...
  Wh(value, trigger) {
    ...
    if (!dragging())
      return;
    dragging.set(false);
    this.Ha.remote.resumeControls(trigger);
  ...
  Xh(event) {
    ...
    const { value, min, max } = this.$state, step = this.Xc.Tc(), keyStep = this.Xc.Uc();
...
class SliderController extends ViewController {
  constructor(_delegate) {
    super();
    this.Xc = _delegate;
    this.Zc = animationFrameThrottle(
  ...
  onSetup() {
    ...
    this.$c();
    new SliderEventsController(this.Xc, this.Ha).attach(this);
  }
  onAttach(el) {
  ...
  Rc() {
    ...
    if (peek(dragging))
      return;
    value.set(getClampedValue(min(), max(), value(), this.Xc.Tc()));
  }
  _c() {
    if (!this.Xc.Wc())
      return;
  ...
  $c() {
    ...
    this.setAttributes({
      ...
      "aria-valuenow": this.Xc.Pc,
      "aria-valuetext": this.Xc.Qc,
      "aria-orientation": orientation
    });
  }
  Yc() {
    const { fillPercent, pointerPercent } = this.$state;
    this.Zc(round(fillPercent(), 3), round(pointerPercent(), 3));
  }
}
...
class Slider extends Component {
  constructor() {
    ...
    new SliderController({
      Tc: this.$props.step,
      Uc: this.$props.keyStep,
      Vc: Math.round,
      Wc: this.$props.disabled,
...
class ThumbnailsLoader {
  ...
  _f() {
    const { canLoad } = this.Ha.$state;
    if (!canLoad())
      return;
    const controller = new AbortController(), { crossorigin } = this.Ha.$state;
    const src = this.$src();
    if (!src)
      return;
    if (cache.has(src)) {
...
let Thumbnail$1 = class Thumbnail extends Component {
  ...
  ld() {
    const { hidden } = this.$state, { duration } = this.Ha.$state, cues = this.Rf.$cues();
  ...
  Tf() {
    const time = this.fd(), { activeCue } = this.$state, { duration } = this.Ha.$state, cues = this.Rf.$cues();
...
class SliderVideo extends Component {
  ...
  kd() {
    const { src } = this.$state, { canLoad } = this.Ha.$state;
  ...
  ld() {
    const { src, hidden } = this.$state, { canLoad, duration } = this.Ha.$state;
  ...
  nd() {
    const { video, canPlay } = this.$state, { duration } = this.Ha.$state, { pointerRate } = this.ed, media = video(), canUpdate = canPlay() && media && Number.isFinite(duration()) && Number.isFinite(pointerRate());
    if (canUpdate) {
...
class VolumeSlider extends Component {
  ...
  onAttach(el) {
    ...
    const { canSetVolume } = this.Ha.$state;
    this.setAttributes({
  ...
  vd(event) {
    ...
    this.Ha.remote.changeVolume(mediaVolume, event);
  }
  ud(event) {
    this.sd(event);
  }
...
class TimeSlider extends Component {
  ...
  onConnect(el) {
    ...
    observeActiveTextTrack(this.Ha.textTracks, "chapters", this.zd.set);
  }
  Fd() {
    const { bufferedEnd, duration } = this.Ha.$state;
  ...
  hc() {
    const { currentTime } = this.Ha.$state, { value, dragging } = this.$state, newValue = this.Id(currentTime());
    if (!peek(dragging)) {
      value.set(newValue);
      this.dispatch("value-change", { detail: newValue });
    }
  ...
  Hd() {
    const player = this.Ha.player.el, { dd: _preview } = useContext(sliderContext);
    player && _preview() && setAttribute(player, "data-preview", this.$state.active());
  }
  yb(time, event) {
    this.Ha.remote.seeking(time, event);
  }
  Jd(time, percent, event) {
    this.Bd.cancel();
    const { live } = this.Ha.$state;
    if (live() && percent >= 99) {
      this.Ha.remote.seekToLiveEdge(event);
      return;
    }
    this.Ha.remote.seek(time, event);
  }
  Ad(event) {
  ...
  // -------------------------------------------------------------------------------------------
  Tc() {
    const value = this.$props.step() / this.Ha.$state.duration() * 100;
    return Number.isFinite(value) ? value : 1;
  }
  Uc() {
    const value = this.$props.keyStep() / this.Ha.$state.duration() * 100;
    return Number.isFinite(value) ? value : 1;
  }
  Vc(value) {
  ...
  Qc() {
    const time = this.wd(this.$state.value()), { duration } = this.Ha.$state;
  ...
  Id(time) {
    const { liveEdge, duration } = this.Ha.$state, rate = Math.max(0, Math.min(1, liveEdge() ? 1 : Math.min(time, duration()) / duration()));
    return Number.isNaN(rate) ? 0 : Number.isFinite(rate) ? rate * 100 : 100;
  }
  Cd(percent) {
    const time = this.wd(percent), { live, duration } = this.Ha.$state;
    return Number.isFinite(time) ? (live() ? time - duration() : time).toFixed(0) : "LIVE";
  }
  Dd(percent, padHours, padMinutes, showHours) {
    const time = this.wd(percent), { live, duration } = this.Ha.$state, value = live() ? time - duration() : time;
...
class SliderChapters extends Component {
  constructor() {
    ...
    this.Qd = signal(-1);
    this.Rd = 0;
    this.Zd = animationFrameThrottle((bufferedPercent) => {
      let percent, cues = this.Md();
      for (let i = this.Rd; i < this.Ld.length; i++) {
        percent = this.Sd(cues[i], bufferedPercent);
    ...
    this._d = computed(this.$d.bind(this));
    this.ae = functionDebounce(
      () => {
        const track = peek(this.Pd);
        if (!this.scope || !track || !track.cues.length)
          return;
        this.Md.set(this.be(track.cues));
        this.Od.set(0);
      },
      150,
      true
  ...
  get activeCue() {
    return this.Md()[this.Od()] || null;
  }
  get activePointerCue() {
    return this.Md()[this.Qd()] || null;
  }
  onSetup() {
    this.Ha = useMediaContext();
  ...
  onAttach(el) {
    observeActiveTextTrack(this.Ha.textTracks, "chapters", this.Vd.bind(this));
    effect(this.ce.bind(this));
  }
  onConnect() {
  ...
  setRefs(refs) {
    ...
    if (this.Ld.length === 1) {
      ...
      scoped(() => this.de(), this.Wd = createScope());
    }
  }
  Vd(track) {
  ...
  ee() {
    let cue, cues = this.Md(), endTime = cues[cues.length - 1].endTime;
  ...
  fe() {
    let { liveEdge, ended } = this.Ha.$state, { fillPercent, value } = this.Ud, cues = this.Md(), isLiveEdge = liveEdge(), prevActiveIndex = peek(this.Od), currentChapter = cues[prevActiveIndex];
    let currentActiveIndex = isLiveEdge ? this.Md.length - 1 : this.Xd(
      currentChapter ? currentChapter.startTime <= peek(value) ? prevActiveIndex : 0 : 0,
      fillPercent()
    );
    if (isLiveEdge || ended() || !currentChapter) {
      this.Td(0, cues.length, "100%");
    } else if (currentActiveIndex > prevActiveIndex) {
      this.Td(prevActiveIndex, currentActiveIndex, "100%");
    } else if (currentActiveIndex < prevActiveIndex) {
      this.Td(currentActiveIndex + 1, prevActiveIndex + 1, "0%");
    }
    const percent = isLiveEdge ? "100%" : this.Sd(cues[currentActiveIndex], fillPercent()) + "%";
    this.Yd(this.Ld[currentActiveIndex], percent);
    this.Od.set(currentActiveIndex);
  }
  ge() {
    ...
    this.Qd.set(activeIndex);
  }
  Td(start, end, percent) {
    for (let i = start; i < end; i++)
      this.Yd(this.Ld[i], percent);
  }
  Yd(ref, percent) {
    ref && ref.style.setProperty("--chapter-fill", percent);
  }
  Xd(startIndex, percent) {
    let chapterPercent = 0, cues = this.Md();
    for (let i = startIndex; i < cues.length; i++) {
      chapterPercent = this.Sd(cues[i], percent);
  ...
  ce() {
    ...
    const track = this.Pd();
    if (track) {
      const onCuesChange = this.ae.bind(this);
      onCuesChange();
...
class MenuFocusController {
  ...
  Ec() {
    if (!this.w)
      return;
    this.Ba();
    listenEvent(this.w, "keyup", this.vb.bind(this));
    listenEvent(this.w, "keydown", this.wb.bind(this));
    onDispose(() => {
      this.jf = 0;
      this.gi = [];
    });
  ...
  Xe(index = this.ji()) {
    const element = this.gi[index], container = this.Xc.Re();
...
class Menu extends Component {
  ...
  onSetup() {
    ...
    this.setAttributes({
      ...
      "data-disabled": this.Wc.bind(this)
    });
  ...
  Me() {
    ...
    if (!expanded)
      return;
    effect(() => {
      const { height } = this.Ha.$state, content = this.yc();
  ...
  we(button) {
    const el = button.el, isMenuItem = this.isSubmenu, isARIADisabled = $ariaBool(this.Wc.bind(this));
  ...
  Je(isExpanded, event) {
    ...
    if (isExpanded) {
      if (!this.isSubmenu && this.Ha.activeMenu !== this) {
        this.Ha.activeMenu?.close(event);
        this.Ha.activeMenu = this;
      }
      this.oe?.Ve?.(event);
    } else {
      if (this.isSubmenu) {
...
class MenuPortal extends Component {
  ...
  _c() {
    const { fullscreen } = this.Ha.$state, { disabled } = this.$props, _disabled = disabled();
...
class ChaptersRadioGroup extends Component {
  ...
  onConnect(el) {
    ...
    effect(this.hc.bind(this));
    effect(this.kf.bind(this));
    observeActiveTextTrack(this.Ha.textTracks, "chapters", this.ta.set);
  }
  Rc() {
  ...
  hc() {
    ...
    const { currentTime } = this.Ha.$state, time = currentTime(), activeCueIndex = track.cues.findIndex((cue) => isCueActive(cue, time));
  ...
  ud(value, trigger) {
    if (this.disabled || !trigger)
      return;
    const index = +value, cues = this.ta()?.cues;
    if (isNumber(index) && cues?.[index]) {
      this.jf.set(index);
      this.Ha.remote.seek(cues[index].startTime, trigger);
...
class AudioRadioGroup extends Component {
  ...
  get disabled() {
    const { audioTracks } = this.Ha.$state;
  ...
  mf() {
    const { emptyLabel } = this.$props, { audioTrack } = this.Ha.$state, track = audioTrack();
  ...
  lf() {
    const { audioTrack } = this.Ha.$state;
  ...
  ud(value, trigger) {
    if (this.disabled)
      return;
    const index = this.Ha.audioTracks.toArray().findIndex((track) => track.label.toLowerCase() === value);
    if (index >= 0) {
      const track = this.Ha.audioTracks[index];
      this.Ha.remote.changeAudioTrack(index, trigger);
...
class CaptionsRadioGroup extends Component {
  ...
  getOptions() {
    const { offLabel } = this.$props, { textTracks } = this.Ha.$state;
    return [
  ...
  mf() {
    const { offLabel } = this.$props, { textTrack } = this.Ha.$state, track = textTrack();
    this.Ye?.Ue.set(
  ...
  ud(value, trigger) {
    if (this.disabled)
      return;
    if (value === "off") {
      const track = this.Ha.textTracks.selected;
      if (track) {
        const index2 = this.Ha.textTracks.toArray().indexOf(track);
        this.Ha.remote.changeTextTrackMode(index2, "disabled", trigger);
        this.dispatch("change", { detail: null, trigger });
      }
      return;
    }
    const index = this.Ha.textTracks.toArray().findIndex((track) => track.label.toLowerCase() === value);
    if (index >= 0) {
      const track = this.Ha.textTracks[index];
      this.Ha.remote.changeTextTrackMode(index, "showing", trigger);
      this.dispatch("change", { detail: track, trigger });
    }
  }
}
...
class SpeedRadioGroup extends Component {
  ...
  mf() {
    const { normalLabel } = this.$props, { playbackRate } = this.Ha.$state, rate = playbackRate();
    this.Ye?.Ue.set(rate === 1 ? normalLabel() : rate + "\xD7");
  }
  lf() {
    const { playbackRate } = this.Ha.$state;
    return playbackRate() + "";
  }
  ud(value, trigger) {
    const rate = +value;
    this.Ha.remote.changePlaybackRate(rate, trigger);
...
class QualityRadioGroup extends Component {
  ...
  get disabled() {
    const { canSetQuality, qualities } = this.Ha.$state;
  ...
  mf() {
    const { autoLabel } = this.$props, { autoQuality, quality } = this.Ha.$state, qualityText = quality() ? quality().height + "p" : "";
  ...
  ud(value, trigger) {
    if (this.disabled)
      return;
    if (value === "auto") {
      this.Ha.remote.changeQuality(-1, trigger);
      this.dispatch("change", { detail: "auto", trigger });
      return;
    }
    const { qualities } = this.Ha.$state, index = peek(qualities).findIndex((quality) => this.nf(quality) === value);
    if (index >= 0) {
      const quality = peek(qualities)[index];
      this.Ha.remote.changeQuality(index, trigger);
      this.dispatch("change", { detail: quality, trigger });
    }
  }
  lf() {
    const { quality, autoQuality } = this.Ha.$state;
    if (autoQuality())
      return "auto";
...
let Gesture$1 = class Gesture extends Component {
  ...
  onConnect(el) {
    this.Rb = this.Ha.player.el?.querySelector(
      "[data-media-provider]"
    );
  ...
  sf() {
    ...
    if (!this.Rb || !eventType)
      return;
    if (/^dbl/.test(eventType)) {
      eventType = eventType.split(/^dbl/)[1];
    }
    if (eventType === "pointerup" || eventType === "pointerdown") {
      const pointer = this.Ha.$state.pointer();
      if (pointer === "coarse") {
  ...
  tf(event) {
    if (isPointerEvent(event) && (event.button !== 0 || this.Ha.activeMenu) || isTouchEvent(event) && this.Ha.activeMenu || isTouchPinchEvent(event) || !this.uf(event)) {
      return;
    }
  ...
  vf() {
    const gestures = this.Ha.player.el.querySelectorAll(
  ...
  wf(action, trigger) {
    ...
    if (action.includes(":fullscreen")) {
      this.Ha.remote.toggleFullscreen("prefer-media", trigger);
    } else if (action.includes("seek:")) {
      this.Ha.remote.seek(peek(this.Ha.$state.currentTime) + (+value || 0), trigger);
    } else {
      this.Ha.remote[kebabToCamelCase(method)](trigger);
    }
...
let Captions$1 = class Captions extends Component {
  ...
  onDestroy() {
    if (this.xf) {
      this.xf.detach();
      this.Ha.textRenderers.remove(this.xf);
    }
    this.yf?.destroy();
  }
  Mc() {
    const { textTrack } = this.Ha.$state, track = textTrack();
    return this.Ha.$iosControls() || !track || !isTrackCaptionKind(track);
  }
  Af() {
    const { viewType } = this.Ha.$state;
    if (viewType() === "audio") {
      return this.Cf();
    } else {
      return this.Df();
  ...
  Bf() {
    this.el.textContent = "";
    const { currentTime, textTrack } = this.Ha.$state, time = peek(currentTime), activeCues = peek(textTrack).activeCues;
  ...
  Ef() {
    const { currentTime } = this.Ha.$state, { updateTimedVTTCueNodes } = this.zf;
    updateTimedVTTCueNodes(this.el, currentTime());
  }
  Df() {
    effect(this.Ff.bind(this));
    effect(this.Gf.bind(this));
    this.Ha.textRenderers.add(this.xf);
    return () => {
      this.el.textContent = "";
      this.xf.detach();
      this.Ha.textRenderers.remove(this.xf);
  ...
  Gf() {
    if (this.Mc())
      return;
    const { currentTime } = this.Ha.$state;
...
class Poster extends Component {
  ...
  onConnect(el) {
    const { canLoad, poster } = this.Ha.$state;
  ...
  ld() {
    const { src } = this.$props, { $iosControls } = this.Ha, { poster } = this.Ha.$state;
  ...
  Hf() {
    const { src: _src } = this.$props, { src } = this.$state, { canLoad, poster } = this.Ha.$state;
    const _poster = _src() || poster();
    if (poster() !== _poster) {
      this.Ha.delegate.Nb("poster-change", {
        detail: _poster
      });
    }
  ...
  Kf() {
    const { loading, error } = this.$state, { canLoad, poster } = this.Ha.$state;
...
let Time$1 = class Time extends Component {
  ...
  Mf() {
    const { type, remainder, padHours, padMinutes, showHours } = this.$props, seconds = this.Nf(type()), duration = this.Ha.$state.duration();
  ...
  Nf(type) {
    const { bufferedEnd, duration, currentTime } = this.Ha.$state;
    switch (type) {
      case "buffered":
        return bufferedEnd();
      case "duration":
